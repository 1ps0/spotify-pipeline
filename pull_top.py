...
def run(event, context):
    ...
    if len(me_last_scans):
        ...
        if (timestamp_now() - me_last_scan) / 86400 < 1: # 1 day threshold
            me_do_scan = False

    if me_do_scan:
        me_artists = api_client.get_personalization('artists')
        local_save_data(f"data/me/artists/{stamp}.json", json.dumps(me_artists))

        me_tracks = api_client.get_personalization('tracks')
        local_save_data(f"data/me/tracks/{stamp}.json", json.dumps(me_tracks))
